...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id, sub_idx, obj_idx, pred_idx):
...
def add_marker_tokens(tokenizer, ner_labels):
    new_tokens = ['<SUBJ_START>', '<SUBJ_END>', '<OBJ_START>', '<OBJ_END>', '<PRED_START>', '<PRED_END>']
    for label in ner_labels:
        ...
        new_tokens.append('<PRED_START=%s>'%label)
        new_tokens.append('<PRED_END=%s>'%label)
    for label in ner_labels:
        new_tokens.append('<SUBJ=%s>'%label)
        new_tokens.append('<OBJ=%s>'%label)
        new_tokens.append('<PRED=%s>' % label)
...
def convert_examples_to_features(examples, label2id, max_seq_length, tokenizer, special_tokens, unused_tokens=True):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        PREDICATE_START = get_special_token("PRED_START")
        PREDICATE_END = get_special_token("PRED_END")

        SUBJECT_NER = get_special_token("SUBJ=%s" % example['subj_type'])
        OBJECT_NER = get_special_token("OBJ=%s" % example['obj_type'])
        PREDICATE_NER = get_special_token("PRED=%s" % example['predicate_type'])
        ...
        PREDICATE_START_NER = get_special_token("PRED_START=%s"%example['predicate_type'])
        PREDICATE_END_NER = get_special_token("PRED_END=%s"%example['predicate_type'])
        ...
        for i, token in enumerate(example['token']):
            ...
            if i == example['predicate_start']:
                pred_idx = len(tokens)
                tokens.append(PREDICATE_START_NER)
            ...
            if i == example['predicate_end']:
                tokens.append(PREDICATE_END_NER)
        tokens.append(SEP)
        ...
        if num_shown_examples < 20:
            if (ex_index < 5) or (label_id > 0):
                ...
                print("sub_idx, obj_idx, predicate_idx: %d, %d %d" % (sub_idx, obj_idx, pred_idx))
...
def evaluate(model, device, eval_dataloader, eval_label_ids, num_labels, e2e_ngold=None, verbose=True):
    ...
    nb_eval_steps = 0
    preds = []
    for input_ids, input_mask, segment_ids, label_ids, sub_idx, obj_idx, pred_idx in eval_dataloader:
        ...
        with torch.no_grad():
            logits = model(input_ids, segment_ids, input_mask, labels=None, sub_idx=sub_idx, obj_idx=obj_idx, pred_idx=pred_idx)
...
def main(args):
    ...
    if args.do_eval and (args.do_train or not(args.eval_test)):
        ...
        all_pred_idx = torch.tensor([f.pred_idx for f in eval_features], dtype=torch.long)
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_sub_idx, all_obj_idx, all_pred_idx)
    ...
    if args.do_train:
        ...
        all_pred_idx = torch.tensor([f.pred_idx for f in train_features], dtype=torch.long)
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_sub_idx, all_obj_idx, all_pred_idx)
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_batches):
                ...
                input_ids, input_mask, segment_ids, label_ids, sub_idx, obj_idx, pred_idx = batch
                loss = model(input_ids, segment_ids, input_mask, label_ids, sub_idx, obj_idx, pred_idx)
                if n_gpu > 1:
                    loss = loss.mean()
